...
class WeatherActivity : AppCompatActivity(), KodeinAware {
    ...
    private fun changeBgAccToTemp(iconCode: String?) {
        when (iconCode) {
            "01d", "02d", "03d" -> {
                dataBind.imageWeatherHumanReaction.setImageResource(R.drawable.sunny_day)
                dataBind.extraDetails.text = "In a sunny day like today you have to drink +1000ml"
            }
            "04d", "09d", "10d", "11d" -> {
                dataBind.imageWeatherHumanReaction.setImageResource(R.drawable.raining)
                dataBind.extraDetails.text = "In a rainny day like today you have to drink +500ml if you need"
            }
            "13d", "50d" -> {
                dataBind.imageWeatherHumanReaction.setImageResource(R.drawable.snowfalling)
                dataBind.extraDetails.text = "In a snowy day like today you don't have to drink extra water"
            }
        }
    }
